(function () {
  ...
  function FlightEngineerPage() {
    ...
    return h("div", { className: "flight-engineer-page" },
      ...
      h(Card, null, h(CardHeader, null, h("div", { className: "flight-engineer-profile-head" }, h("div", null, h(CardTitle, null, "Live inference"), h("div", { className: "flight-engineer-stat-label" }, telemetry ? "Updates every 2 seconds" : "Telemetry unavailable")), h("div", { className: "flight-engineer-live-copy" }, number(inf.lifetime_tokens_per_second) + " tok/s lifetime · " + number(inf.queued) + " queued"))),
        h(CardContent, null, h("div", { className: "flight-engineer-meters" }, h(LevelMeter, { label: "Inference slots", percent: slotPercent, value: number(inf.active) + " / " + number(inf.slots) }), h(LevelMeter, { label: "GPU", percent: sys.gpu_percent, value: number(sys.gpu_percent) + "%" }), h(LevelMeter, { label: "VRAM", percent: vramPercent, value: number(sys.vram_used_mb) + " / " + number(sys.vram_total_mb) + " MB" }), h(LevelMeter, { label: "Host RAM", percent: ramPercent, value: number(sys.ram_used_mb) + " / " + number(sys.ram_total_mb) + " MB" }), h(LevelMeter, { label: "CPU load", percent: sys.cpu_load_percent, value: number(sys.cpu_load_percent) + "%" }), h(LevelMeter, { label: "GPU power", percent: powerPercent, value: number(sys.gpu_power_w) + " W" })))),
